...
nltk.data.path.append('/Users/anshikaverma/nltk_data')  # <- Replace with your real path
...
def solve_math_equation(prompt):
    ...
    try:
        ...
        return "Sorry, I couldn't solve that. Try writing it like '2*x + 3 - 7'"

# Load the intents.json file
with open("/Users/aarikarathod/PyCharmMiscProject/chatbot/intents.json", "r") as file:
...
def send_message():
    ...
    if tag == "math_help":
        chat_log.insert(tk.END, "Laufey: " + random.choice(
            [r for i in intents["intents"] if i["tag"] == "math_help" for r in i["responses"]]) + '\n')
        chat_log.insert(tk.END, "Laufey: " + solve_math_equation(user_input) + '\n')
    else:
        for intent in intents["intents"]:
            if intent["tag"] == tag:
                chat_log.insert(tk.END, "Laufey: " + random.choice(intent["responses"]) + '\n')
                break

# Launch GUI
root = tk.Tk()
root.title("Laufey with essay help")

chat_log = scrolledtext.ScrolledText(root, width=130, height=40, bg="light pink")
chat_log.pack()
